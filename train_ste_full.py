...
from layerwise_ste_tuning import wrap_model_ste, unwrap_model_ste, save_codebook_layers, CodebookLoRASTELinear

from layerwise_ste_tuning import cleanup


def set_trainable(model: nn.Module, lr: float=1e-3) -> list[nn.Parameter]:
    """
    ...
    for name, param in model.named_parameters():
        if "codebook" in name:
            ...
            codebook_to_train.append(param)
        elif "scale" in name:
            param.requires_grad = True
    ...
    param_groups = [
        {"params": codebook_to_train, "lr": lr, "label": "codebook"},
        {"params": scales_to_train, "lr": 10 * lr, "label": "scale"},
        #{"params": lora_to_train, "lr": lora_lr, "label": "lora"},
        {"params": lora_to_train, "lr": 0.1 * lr, "label": "lora"},
    ]
    return param_groups
...
def get_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Group size for compression for 2bit codebooks. 4bit codebooks will have 2 * group_size",
    )
...
def main(argv) -> float:
    ...
    param_to_train = set_trainable(model, lr=args.lr)
