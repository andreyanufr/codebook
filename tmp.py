...
def _ste_recompute_fn(
    ...
    """STE dequantisation with gather-based hard path (optimal for 4-bit).

    Called inside ``torch.utils.checkpoint`` so that the large intermediate
    tensors (``soft_assignment``, ``neg_dist_sq`` – both shape
    ``(out, in//gs, gs, 2^n_bits)``) are discarded after forward and
    **recomputed** during backward, saving significant GPU memory.

    The hard-assignment path uses a direct gather ``codebook[indexes]``
    instead of building a full one-hot tensor.  For 4-bit codebooks this
    avoids materialising a tensor 16× the weight size.
    """
    codebook = codebook_raw / codebook_raw.abs().max().clamp(min=1e-8)

    effective_weight = orig_weight + (lora_B @ lora_A) * (lora_alpha / lora_rank)
    effective_weight_grouped = effective_weight.view(
        out_features, in_features // group_size, group_size
    )

    if use_exp_for_scale:
        scale = scale_raw.exp()
    else:
        scale = scale_raw
    ...
    soft_weight = (codebook * soft_assignment).sum(dim=-1)

    # Hard weight via direct gather – avoids the large one-hot tensor
    # (especially beneficial for 4-bit where one-hot would be 16× weight size)
    hard_weight = codebook[indexes.long()]

    # STE trick: forward value = hard, backward gradient = soft
    weight = (hard_weight + (soft_weight - soft_weight.detach())) * scale
...
class CodebookLoRASTELinear(nn.Module):
    ...
    def _dequantize_ste(self):
        """STE dequantisation with memory-efficient recomputation.

        Uses ``torch.utils.checkpoint`` to wrap :func:`_ste_recompute_fn`
        so that the large intermediate tensors (``soft_assignment``,
        ``neg_dist_sq``) are discarded after the forward pass and
        recomputed only during backward.

        The hard-assignment path uses a direct ``codebook[indexes]`` gather
        instead of a full one-hot tensor, which is especially efficient for
        4-bit codebooks (avoids a 16×-weight-size intermediate).

        The forward result is identical to standard STE dequantisation.
        """
        out_features, in_features = self.orig_layer.weight.shape
        if self.n_bits > 2 or max(out_features, in_features) > 4096 or True:
            ...
            # For 2-bit codebooks, the memory savings from checkpointing are smaller,
            # and the overhead of recomputation can dominate.  In this case we can
            # skip checkpointing and compute the STE dequantisation directly.
            return _ste_recompute_fn(
                self.codebook,
                self.scale,
                self.lora_B,
                self.lora_A,
    ...
    for epoch in range(epochs_per_layer):
        ...
        for indices in track(
            ...
            if grad_steps == grad_accumulation_steps:
                ...
                if epoch < index_update_epochs * 10000:
                    for m in ste_modules:
                        m.update_indexes()
