...
from train_layerwise import wrap_model_ste, unwrap_model_ste, CodebookLoRASTELinear, save_codebook_layers
...
def set_trainable(model: nn.Module) -> list[nn.Parameter]:
    ...
    for name, param in model.named_parameters():
        if "codebook" in name:
            ...
            param.requires_grad = True
            scales_params.append(param)
        elif "lora" in name:
            param.requires_grad = True
            lora_params.append(param)
        else:
            param.requires_grad = False
    return scales_params, codebook_params, lora_params
...
def main(argv) -> float:
    ...
    if args.layerwise:
        ...
        finetune_layerwise_ste(model, train_loader, lr=args.lr,
    ...
    # args.num_train_samples, seqlen=args.train_seqlen
    hiddens_dir = output_dir / f"hiddens_{args.train_seqlen}_{args.num_train_samples}.pt"
    ...
    opt = torch.optim.AdamW([
        {"params": scales_params, "lr": 10 * args.lr},
        {"params": codebook_params, "lr": args.lr},
        {"params": lora_params, "lr": 0.1 * args.lr},
    ])
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, eta_min=args.lr * 1e-4, T_max=total_opt_steps)
